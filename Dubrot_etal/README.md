...
**Figure 2**: Quality control and exploratory data analysis of RNA-seq data.
**Panel A–B**: Raw and VST-normalized transcript count distribution
**Panel C–D**: Raw and VST-normalized relative log expression (RLE)
**Panel E-F**: Principal component analysis (PCA) of raw and VST-normalized transcript counts
These plots highlight the effect of normalization in reducing technical variation and stabilizing variance across samples.
...
The following script performs differential gene expression (DGE) analysis using DESeq2 and subsequent functional enrichment analysis using clusterProfiler. Each cell line is analyzed separately to identify treatment-dependent changes in gene expression.

Script: scripts/02_downstream_analysis/03_differential_abundance_analysis.R
**Script:** [`scripts/02_downstream_analysis/03_differential_abundance_analysis.R`](scripts/02_downstream_analysis/03_differential_abundance_analysis.R)
...
# Date: May 29th, 2025
# Author: MA Yaqin
# Description: Conduct the differential abundance analysis 
...
# ===== Data import =====
txi <- readRDS("result/transcript_abundance.rds")
tx2gene <- readRDS("result/transcript_annotation.rds")
metadata <- readRDS("result/metadata.rds")

dds <- DESeqDataSetFromTximport(txi, colData = metadata, design = ~ cell_line)
cell_list <- unique(metadata$cell_line)

```

This script preprocess the data such as:
1. Data import
  - Reads in transcript quantification (txi), transcript–gene mapping (tx2gene), and experimental metadata.
  - Builds a DESeq2 dataset (dds) where cell_line is the main design variable.

2. Parameter setting
 - fdr = 0.05: cutoff for adjusted p-values (Benjamini–Hochberg).
 - lfc = 0.5: minimum log2 fold-change considered biologically relevant.

3. Gene annotation
 - Adds Entrez IDs to gene symbols via org.Mm.eg.db.
 - Keeps only genes with successful mapping (important for downstream enrichment like GO).

**Experimental design setup**
The design formula is set to ~ treatment, meaning **treatment condition** is the explanatory variable of interest. In this context, **IFNg** is specified as the reference level, so DESeq2 will test differential expression between IFNg-treated and NS (control) groups.

This setup ensures that DESeq2 models gene expression as a function of treatment status, allowing us to identify genes significantly up- or downregulated under IFNg stimulation compared to baseline.

**Differential expression analysis**
For each cell line, datasets were subset and the design was re-defined as ~ treatment.
This allows testing the effect of IFNg treatment vs NS (control) independently within each cell type.

Steps within the loop:

1. Run DESeq2
  - The differential expression is calculated with results(dds_sub, contrast = c("treatment", "IFNg", "NS")).
  - Genes are tested for significance using the specified thresholds (FDR < 0.05, |log2FC| ≥ 0.5).

2. Result exploration & visualization
  - MA Plot: Plots mean expression vs log2 fold change, highlighting significant genes.
  - Volcano Plot: Visualizes the tradeoff between effect size (log2FC) and statistical significance (–log10(padj)), with immune-relevant genes (e.g., Tap1, B2m) highlighted.
  - Top DE genes: A bar plot of the top 10 up- and down-regulated genes per cell line, colored by treatment direction.

3. Enrichment analysis
  - Background (all tested genes), upregulated, and downregulated genes are annotated with Entrez IDs.
  - GO Biological Process enrichment is performed using clusterProfiler::enrichGO.
  - Results are saved as .tsv tables, and top terms are visualized with dot plots.

4. Summary plots
  - Per-cell-line figures combine volcano plots, top DE gene barplots, and enrichment dotplots into one panel (A–D).
  - Exported in .png, .jpeg, and .tiff formats.

5. Gene lists
  - Common background, upregulated, and downregulated genes across cell lines are stored as .rds objects for downstream meta-analysis.
...
Raw counts (tximport)
  ...
  DESeq2 (~ treatment)
        ...
        │
        └──> Enrichment:
               • Upregulated genes → GO BP
               • Downregulated genes → GO BP
...
Following is the example result from YUMMER1.7 cell line.
![DE_resultYUMMER1.7](result/cell_line/YUMMER1.7/DE_resultYUMMER1.7.jpeg "DE_resultYUMMER1.7")

**Figure 3**: Differential expression analysis between IFNg- and non-stimulated **YUMMER1.7** cell line.  
- **Panel A**: Volcano plot highlighting upregulated genes in IFNg and non-stimulated group, colored in green and orange, respectively. Interferon marker genes are also annotated.  
- **Panel B**: Bar plot showing the magnitude of differential expression (log2 FC), with green and orange representing IFNg and non-stimulated groups, subsequently.  
- **Panel C–D**: Dotplots showing enriched Gene Ontology – Biological Process (GO:BP): (**C**) IFNg and (**D**) non-stimulated group. The x-axis lists the genes, while the y-axis depicts the gene ratio (number of genes found relative to the total gene set). Circle size encodes the number of genes; color encodes the adjusted p-value.  

Overall, higher activity of immune response is observed in IFNg cells, while tissue generation processes are apparent in non-stimulated cells. Notably, angiogenesis-related genes — commonly upregulated in cancer cells — were enriched in the control group. This pattern is consistent across cell lines, suggesting a conserved upregulation trend following IFNg stimulation. To test this hypothesis, further study is conducted especially on the upregulation of all cell line. 


---
### 4. Upregulated DE genes analysis
The following script performs pathway-level analysis on the **upregulated differentially expressed (DE) genes across all cell lines**. Specifically, Reactome pathway enrichment was tested, and results are summarized using a dot plot (magnitude and significance of enrichment) and an emapplot (network-like visualization of pathway similarity).

Script: scripts/02_downstream_analysis/04_common_genes_analysis.R
**Script:** [`scripts/02_downstream_analysis/04_common_genes_analysis.R`](scripts/02_downstream_analysis/04_common_genes_analysis.R)
...
Following are the resulting figures
![Upregulated_genes_Reactome_pathways_dotplot](result/Upregulated_genes_Reactome_pathways_dotplot.jpeg "Upregulated_genes_Reactome_pathways_dotplot")
**Figure 4A**: Dot plot showing enriched Reactome pathways in all cell lines. While the y-axis lists the pathways, detailed information on the cell lines can be seen in the x-axis. The dot size reflects the gene ratio of the corresponding enrichment while the color represents adjusted p-value.

![Upregulated_genes_Reactome_pathways_map](result/Upregulated_genes_Reactome_pathways_map.jpeg "Upregulated_genes_Reactome_pathways_map")
**Figure 4B**: Enrichment map of reactome pathways. Each node represent the enriched reactome pathway with it size shows the gene count. Edges indicate functional similarity or gene overlap, providing information on the functional clusters.

From these figures, we can have several take-home messages:
- YUMMER1.7 upregulation of programmed cell death align with its previously reported immunogenic properties. Often referred as "immunologically hot", it is characterized by high T-cells infiltration and higher response to immune checkpoint therapy (Zhang et al., 2022). Additionally, its upregulation of TFNR1 signalling further emphasize its increased apoptosis activity in response to the given immune checkpoint treatment (Ababneh et al., 2024).
